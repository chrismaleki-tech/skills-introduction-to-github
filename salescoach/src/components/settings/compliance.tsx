...
export function RetentionForm({
  ...
  async function save(runSweep = false) {
    ...
    setMsg(runSweep ? `Sweep done — calls ${data.result?.callsCleared ?? 0}, emails ${data.result?.emailsCleared ?? 0}.` : "Saved.");
    startTransition(() => router.refresh());
  }
  ...
  return (
    <div className="space-y-3 text-sm">
      ...
      <label className="flex items-center gap-2">
        <input
          type="checkbox"
          checked={state.redactPiiInEmailBodies}
          onChange={(e) => setState((s) => ({ ...s, redactPiiInEmailBodies: e.target.checked }))}
        />
        Redact PII in outbound email bodies before grading
      </label>
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <label className="block">
          <span className="text-xs text-muted uppercase tracking-wider">Retain email bodies (days, 0=forever)</span>
          <input
            type="number"
            min={0}
            value={state.retainEmailDays}
            onChange={(e) => setState((s) => ({ ...s, retainEmailDays: Number(e.target.value) }))}
            className="mt-1 w-full rounded-lg border border-line bg-surface-2 px-3 py-2"
          />
        </label>
      </div>
