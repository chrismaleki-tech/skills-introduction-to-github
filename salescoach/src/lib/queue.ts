...
export type JobType = "PROCESS_CALL" | "GRADE_ROLEPLAY" | "GRADE_EMAIL" | "RETENTION_SWEEP";
...
async function runJob(jobId: string) {
  ...
  try {
    ...
    } else if (job.type === "GRADE_EMAIL") {
      const { gradeOutboundEmail } = await import("./email-grade");
      await gradeOutboundEmail(String(payload.messageId));
