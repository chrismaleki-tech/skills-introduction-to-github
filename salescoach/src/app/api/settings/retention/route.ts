...
export async function POST(req: Request) {
  ...
  const policy: RetentionPolicy = {
    ...
    redactPiiInEmailBodies:
      typeof body.redactPiiInEmailBodies === "boolean"
        ? body.redactPiiInEmailBodies
        : current.redactPiiInEmailBodies,
    retainCallDays:
      typeof body.retainCallDays === "number" ? Math.max(0, Math.round(body.retainCallDays)) : current.retainCallDays,
    retainEmailDays:
      typeof body.retainEmailDays === "number"
        ? Math.max(0, Math.round(body.retainEmailDays))
        : current.retainEmailDays,
  };
